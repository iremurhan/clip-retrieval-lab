...
# Uses the unified scripts/train/train.slurm.
#
# Usage:
#   bash scripts/train/launch_ablation.sh --run <RUN_ID> --seed <SEED> --config <CONFIG> [-- KEY=VAL ...]
#
# Example:
#   bash scripts/train/launch_ablation.sh --run B0 --seed 42 --config configs/config_flickr30k.yaml
#
# Optional trailing overrides (after `--`) are passed verbatim to run.py's --override:
#   bash scripts/train/launch_ablation.sh --run B0v2 --seed 42 --config configs/config_flickr30k.yaml \
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Usage: bash scripts/train/launch_ablation.sh --run <RUN_ID> --seed <SEED> --config <CONFIG> [-- KEY=VAL ...]"
            exit 1
            ;;
    esac
done
...
# Build the extra args to forward to train.slurm → run.py
FORWARD_ARGS=(--seed "$SEED")
if [ ${#EXTRA_OVERRIDES[@]} -gt 0 ]; then
    FORWARD_ARGS+=(--override "${EXTRA_OVERRIDES[@]}")
fi
